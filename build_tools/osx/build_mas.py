...
def patch_info_plist(config, args):
  ...
  if args.team_id:
    replaces.append(('NWTeamID', args.team_id))
  ...
  for file in info_plist_files:
    if 'nwjs Framework' in file:
      ...
      print('Cannot patch unknown Info.plist %s' % file)
      continue
    info(file)
...
def codesign_app(config, args):
  ...
  system('codesign -f --timestamp --options=runtime --verbose -s "%s" --entitlements %s --deep "%s"' % (identity, tmp_child_entitlements, framework))
  helperApp = glob(args.output, 'nwjs Helper.app', returnOnFound=True)
  system('codesign -f --timestamp --options=runtime --verbose -s "%s" --entitlements %s --deep "%s"' % (identity, tmp_child_entitlements, helperApp))
  for dylib in glob(args.output, '*.dylib'):
    system('codesign -f --timestamp --options=runtime --verbose -s "%s" --entitlements %s --deep "%s"' % (identity, tmp_child_entitlements, dylib))
  for nodebin in glob(args.output, '*.node'):
    system('codesign -f --timestamp --options=runtime --verbose -s "%s" --entitlements %s --deep "%s"' % (identity, tmp_child_entitlements, nodebin))
  ...
  system('codesign -f --timestamp --options=runtime --verbose -s "%s" --entitlements %s --deep "%s"' % (identity, tmp_parent_entitlements, args.output))
...
def main():
  ...
  parser.add_argument('-A', '--application-identity', default=None, help='application identity for codesign.')
  parser.add_argument('-D', '--installer-identity', default=None, help='developer identity for codesign.')
  parser.add_argument('-T', '--team-id', default=None, help='team id for codesign.')
